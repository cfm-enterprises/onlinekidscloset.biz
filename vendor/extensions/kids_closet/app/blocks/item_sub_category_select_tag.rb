class ItemSubCategorySelectTag < FieldTag
  ...
  def field_html
    ...
    select_tag(
      formatted_name,
      options_for_select(ConsignorInventory.sub_category_array(category), value),
      attributes.merge({ :tabindex => tab_index, :class => class_style, :id => field_id })
    )
  end

end
Liquid::Template.register_tag('item_sub_category_select', ItemSubCategorySelectTag)